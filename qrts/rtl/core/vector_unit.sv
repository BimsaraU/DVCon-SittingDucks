...
//
// Source: DVCon_2026/DVCon_SoC_SRC/ACCELERATOR_IP/vector_unit.sv
//
...
#(
  ...
  // These were `localparam` in this list, which is a SystemVerilog-2009
  // construct. Quartus only offers SystemVerilog_2005 for
  // VERILOG_INPUT_VERSION and rejects it outright:
  //   Error (10170): syntax error near "localparam"; expecting an identifier
  //
  // ACT_W and ACC_W size the ports below, so they cannot simply move into the
  // module body. Declaring them as `parameter` is the portable form -- they
  // are still overridable in principle, but nothing overrides them and the
  // widths are fixed by the INT8 datapath.
  ...
  // Activation codes. In the module body rather than the parameter list: they
  // do not size any port, and `localparam` is only legal here under SV-2005.
