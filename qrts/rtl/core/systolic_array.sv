...
//
// Source: DVCon_2026/DVCon_SoC_SRC/ACCELERATOR_IP/systolic_array.sv
//
...
module systolic_array #(
    ...
    genvar gd;
    // The loop body needs an explicit named begin...end block. Vivado accepts
    // a bare `assign` as a generate-loop body; Quartus rejects it outright
    // (Error 10170, "expecting begin"), and a bare `assign` sitting directly
    // inside `generate` alongside a loop confuses its parser for the rest of
    // the file -- one construct here produced eleven cascading syntax errors.
    assign en_sked[0] = en;
    generate
        for (gd = 1; gd <= DIAG_DEPTH; gd = gd+1) begin : g_en_sked
